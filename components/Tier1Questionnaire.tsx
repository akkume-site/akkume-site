...
const roles = ['Decision-maker', 'Influencer', 'Research / Observer'] as const;
...
type Tier1Form = {
  ...
  industry: string;
  role_level: string;
  biggest_barriers: string[];
  reserve_improves_resilience: string;
  reserve_as_closing_cost: string;
  support_fhfa_pilot: string;
  consider_pilot_participation: string;
...
export default function Tier1Questionnaire() {
  ...
  const toggleBarrier = (item: string) => {
    ...
    if (current.includes(item)) {
      setForm({
        ...form,
        biggest_barriers: current.filter((x) => x !== item),
      });
      return;
    }
  ...
  if (tier1Id) {
    return (
      <div className="space-y-4 rounded-3xl border border-neutral-200 bg-white p-6">
        ...
        <Link
          href={`/questionnaire?tier=2&tier1=${tier1Id}`}
          className="inline-flex min-h-11 items-center justify-center rounded-full bg-neutral-900 px-5 py-3 text-sm font-medium text-white"
        >
          Continue to Tier 2
        </Link>
      </div>
  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-3xl border border-neutral-200 bg-white p-6">
      ...
      <input
        className="min-h-11 w-full rounded-xl border px-4 py-3"
        ...
        {roles.map((item) => (
          <label key={item} className="flex items-center gap-2 rounded-xl border p-3 text-sm">
            <input
              type="radio"
              name="role_level"
              checked={form.role_level === item}
              onChange={() => setForm({ ...form, role_level: item })}
            />
            <span>{item}</span>
          </label>
        ))}
      ...
      <div className="grid gap-2">
        {barriers.map((item) => (
          <label key={item} className="flex items-center gap-2 rounded-xl border p-3 text-sm">
            <input
              type="checkbox"
              checked={form.biggest_barriers.includes(item)}
              onChange={() => toggleBarrier(item)}
            />
            <span>{item}</span>
          </label>
        ))}
      ...
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          checked={form.is_public_supporter}
          onChange={(e) => setForm({ ...form, is_public_supporter: e.target.checked })}
        />
